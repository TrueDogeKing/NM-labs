function [matrix_sizes, condition_numbers, interpolation_error_exact, interpolation_error_perturbed] = ...
    ...
    if ~isempty(threshold_index)
        ...
        xline(size_threshold, '--', 'cond(V) > 10^8', 'LabelOrientation', 'horizontal', ...
            'LabelVerticalAlignment', 'top', 'LabelHorizontalAlignment', 'left', 'Color', [0.494 0.184 0.556]);
    end
    ...
    % Wykres 3
    nexttile;
    semilogy(matrix_sizes, interpolation_error_perturbed, 'g-o');
    grid on;
    xlabel('Rozmiar macierzy');
    ylabel('Maksymalny błąd');
